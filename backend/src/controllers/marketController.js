  ...
  try {
    const results = await Promise.all(
      INDICES.map(async (idx) => {
        try {
          const quote = await getQuoteWithFallback(idx.ticker);
          return {
            name: idx.name,
            ticker: idx.ticker,
            price: quote.price,
          ...
          return null; // skip silently if an index symbol doesn't resolve
        }
      })
    );

    res.status(200).json({ indices: results.filter(Boolean) });
  } catch (err) {
    console.error("Get indices error:", err.message);
    res.status(500).json({ message: "Server error fetching indices" });
  }
};

// @route  GET /api/market/movers?type=gainers|losers
const getMovers = async (req, res) => {
  try {
    const { type } = req.query;

    const withQuotes = await Promise.all(
      FALLBACK_POOL.map(async (p) => {
        try {
    ...
    const clean = withQuotes.filter(Boolean);
    clean.sort((a, b) =>
      type === "losers"
    ...
    res.status(200).json({ movers: clean.slice(0, 10) });
  } catch (err) {
    console.error("Get movers error:", err.message);
    res.status(500).json({ message: "Server error fetching movers" });
  }
};
...
module.exports = { getQuote, search, getPopular, getTickerHistory, getIndices, getMovers };